...
6. **缓存防击穿**：活动详情采用**逻辑过期**策略 —— 缓存里同时保存逻辑过期时间戳，
   到期后由后台线程异步重建，**当前请求立即返回旧值而不阻塞**。
   选它而非互斥锁的理由：活动详情是「变化极少、可容忍短暂旧值」的静态元数据，
   牺牲一点一致性换可用性更划算（互斥锁方案下并发请求会阻塞等待重建完成）。
   配套：Redisson 锁（看门狗续期，防重复重建与锁误删）、空值占位防穿透、TTL 抖动防雪崩。
