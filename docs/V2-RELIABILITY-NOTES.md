...
> 📌 **后续变更（v3.0.0 之后）**：该实现已被移除。
> 原因是修复了「逻辑过期策略未被注入」之后，它变成了新的死代码 ——
> 项目只有一个缓存读路径，为它保留两套防击穿实现属于过度设计。
> 本节记录的修复经验（Redisson 锁、空值占位、有界等待）**同样适用于现存的
> `LogicalExpCacheService`**，两者共用同一套处理思路。
> 若需查阅原实现，见 `v3.0.0` 标签。
